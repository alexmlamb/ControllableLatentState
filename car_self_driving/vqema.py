...
class VectorQuantizerEMA(nn.Module):
    ...
    def forward(self, inputs, reinit_code=None):
        ...
        #distances *= torch.sqrt(self._ema_cluster_size.unsqueeze(0).repeat(distances.shape[0], 1)) #was using        
        
        distances *= torch.sqrt(torch.sqrt(torch.sqrt(self._ema_cluster_size.unsqueeze(0).repeat(distances.shape[0], 1))))
